...
//
// SplitterImpl<T>
// SplitterWindowImpl<T, TBase, TWinTraits>
// SplitterWindowT
...
/////////////////////////////////////////////////////////////////////////////
// SplitterImpl - Provides splitter support to any window
...
// Attributes
	...
	bool SetSplitterPos(int xyPos = -1, bool bUpdate = true)
	{
		...
		{
			if(m_bVertical)
				xyPos = (m_rSplitter.right - m_rSplitter.left - m_cxySplitBar - m_cxyBarEdge) / 2;
			else
				xyPos = (m_rSplitter.bottom - m_rSplitter.top - m_cxySplitBar - m_cxyBarEdge) / 2;
		}

		// Adjust if out of valid range
		int cxyMax = 0;
		if(m_bVertical)
			cxyMax = m_rSplitter.right - m_rSplitter.left;
		else
			cxyMax = m_rSplitter.bottom - m_rSplitter.top;
	...
	DWORD SetSplitterExtendedStyle(DWORD dwExtendedStyle, DWORD dwMask = 0)
	{
		...
		if(IsProportional() && IsRightAligned())
			ATLTRACE2(atlTraceUI, 0, "SplitterImpl::SetSplitterExtendedStyle - SPLIT_PROPORTIONAL and SPLIT_RIGHTALIGNED are mutually exclusive, defaulting to SPLIT_PROPORTIONAL.\n");
#endif //_DEBUG
		return dwPrevStyle;
	}
	...
	void DrawSplitterBar(CDCHandle dc)
	{
		...
		if(GetSplitterBarRect(&rect))
		{
			...
			else
			{
				...
				c = RGB(max(0, GetRValue(c) - nDarken), max(  0, GetGValue(c) - nDarken), max(0, GetBValue(c) - nDarken));
...
// Message map and handlers
	typedef SplitterImpl< T >	thisClass;
	...
	LRESULT OnMouseMove(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if((wParam & MK_LBUTTON) && ::GetCapture() == pT->m_hWnd)
		{
			int xyNewSplitPos = 0;
			if(m_bVertical)
				xyNewSplitPos = xPos - m_rSplitter.left - m_cxyDragOffset;
			else
				xyNewSplitPos = yPos - m_rSplitter.top - m_cxyDragOffset;
	...
	LRESULT OnLButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
	{
		...
		if(IsOverSplitterBar(xPos, yPos))
		{
			...
			if(!m_bFullDrag)
				DrawGhostBar();
			if(m_bVertical)
				m_cxyDragOffset = xPos - m_rSplitter.left - m_xySplitterPos;
			else
				m_cxyDragOffset = yPos - m_rSplitter.top - m_xySplitterPos;
		}
		bHandled = FALSE;
		return 1;
	}
...
// Implementation - internal helpers
	...
	bool GetSplitterBarRect(LPRECT lpRect) const
	{
		...
		if(m_bVertical)
		{
			lpRect->left = m_rSplitter.left + m_xySplitterPos;
			lpRect->top = m_rSplitter.top;
			lpRect->right = m_rSplitter.left + m_xySplitterPos + m_cxySplitBar + m_cxyBarEdge;
			lpRect->bottom = m_rSplitter.bottom;
		}
		else
		{
			lpRect->left = m_rSplitter.left;
			lpRect->top = m_rSplitter.top + m_xySplitterPos;
			lpRect->right = m_rSplitter.right;
			lpRect->bottom = m_rSplitter.top + m_xySplitterPos + m_cxySplitBar + m_cxyBarEdge;
		}
	...
	bool GetSplitterPaneRect(int nPane, LPRECT lpRect) const
	{
		...
		else if(nPane == SPLIT_PANE_LEFT)
		{
			if(m_bVertical)
			{
				lpRect->left = m_rSplitter.left;
				lpRect->top = m_rSplitter.top;
				lpRect->right = m_rSplitter.left + m_xySplitterPos;
				lpRect->bottom = m_rSplitter.bottom;
			}
			else
			{
				...
				lpRect->top = m_rSplitter.top;
				lpRect->right = m_rSplitter.right;
				lpRect->bottom = m_rSplitter.top + m_xySplitterPos;
			}
		}	
		else if(nPane == SPLIT_PANE_RIGHT)
		{
			if(m_bVertical)
			{
				lpRect->left = m_rSplitter.left + m_xySplitterPos + m_cxySplitBar + m_cxyBarEdge;
				lpRect->top = m_rSplitter.top;
				lpRect->right = m_rSplitter.right;
				lpRect->bottom = m_rSplitter.bottom;
			}
			...
			{
				lpRect->left = m_rSplitter.left;
				lpRect->top = m_rSplitter.top + m_xySplitterPos + m_cxySplitBar + m_cxyBarEdge;
				lpRect->right = m_rSplitter.right;
				lpRect->bottom = m_rSplitter.bottom;
			}
		}
	...
	{
		// -1 == don't check
		return ((x == -1 || (x >= m_rSplitter.left && x <= m_rSplitter.right)) &&
			(y == -1 || (y >= m_rSplitter.top && y <= m_rSplitter.bottom)));
	}
	...
	bool IsOverSplitterBar(int x, int y) const
	{
		...
		int xyOff = m_bVertical ? m_rSplitter.left : m_rSplitter.top;
	...
	{
		int cxyTotal = m_bVertical ? (m_rSplitter.right - m_rSplitter.left - m_cxySplitBar - m_cxyBarEdge) : (m_rSplitter.bottom - m_rSplitter.top - m_cxySplitBar - m_cxyBarEdge);
		if(cxyTotal > 0)
			m_nProportionalPos = ::MulDiv(m_xySplitterPos, m_nPropMax, cxyTotal);
		else
			m_nProportionalPos = m_nPropMax / 2;
		ATLTRACE2(atlTraceUI, 0, "SplitterImpl::StoreProportionalPos - %i\n", m_nProportionalPos);
	}
	...
	void SetProportionalPos(int nPos)
	{
		...
		T* pT = static_cast<T*>(this);
		if (pT->m_hWnd) UpdateSplitterLayout();
	}
	...
	void StoreRightAlignPos()
	{
		...
		if(cxyTotal > 0)
			...
			m_nProportionalPos = m_nPropMax / 2;
		ATLTRACE2(atlTraceUI, 0, "SplitterImpl::StoreRightAlignPos - %i\n", m_nProportionalPos);
	}
...
/////////////////////////////////////////////////////////////////////////////
// SplitterWindowImpl - Implements a splitter window

template <class T, class TBase = CWindow, class TWinTraits = CControlWinTraits>
class ATL_NO_VTABLE SplitterWindowImpl : public CWindowImpl< T, TBase, TWinTraits >, public SplitterImpl< T >
{
public:
	DECLARE_WND_CLASS_EX(NULL, CS_DBLCLKS, COLOR_WINDOW)

	typedef SplitterWindowImpl< T , TBase, TWinTraits >	thisClass;
	typedef SplitterImpl< T >				baseClass;
	BEGIN_MSG_MAP(thisClass)
...
/////////////////////////////////////////////////////////////////////////////
// SplitterWindow - Implements a splitter window to be used as is

class SplitterWindow : public SplitterWindowImpl<SplitterWindow>
{
public:
